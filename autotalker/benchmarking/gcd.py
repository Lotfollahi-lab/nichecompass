...
def compute_avg_gcd(
    ...
    ´adata.obsp[f'autotalker_spatial_{n_neighbors}nng']´ and
    ´adata.obsp[f'autotalker_latent_{n_neighbors}nng']´ respectively.
    ...
    adata:
        ...
        ´adata.obsp[f'autotalker_spatial_{n_neighbors}nng']´ and
        ´adata.obsp[f'autotalker_latent_{n_neighbors}nng']´.
    spatial_key:
...
def compute_gcd(
    ...
    neighbor graph and the spatial nearest neighbor graph. If provided and
    existent, use precomputed nearest neighbor graphs stored in
    ...
    connectivities_diff = (adata.obsp[latent_knng_key] -
                           adata.obsp[spatial_knng_key]).toarray()
