...
def main(args):
    ...
    if args.dataset == "deeplinc_seqfish":
        adata = load_spatial_adata_from_csv("datasets/seqFISH/counts.csv", "datasets/seqFISH/adj.csv")
    ...
    print(f"Average number of neighbours: {adata.obsp['spatial_connectivities'].toarray().sum(axis=0).mean()}")
    print(f"Number of nodes: {adata.X.shape[0]}")
    print(f"Number of node features: {adata.X.shape[1]}")
    print(f"Number of edges: {int(np.triu(adata.obsp['spatial_connectivities'].toarray()).sum())}", sep="")

    model = Autotalker(adata, n_hidden = args.n_hidden, n_latent = args.n_latent, dropout_rate = args.dropout_rate)
    model.train()
